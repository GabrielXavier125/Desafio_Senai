<div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-5">
        <div class="relative w-full sm:w-72">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar empresa ou CNPJ..."
        ...
        <table class="w-full text-sm min-w-[680px]">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="text-left px-5 py-3 font-semibold text-gray-600">CNPJ</th>
                    <th class="text-left px-5 py-3 font-semibold text-gray-600">E-mail de acesso</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($empresas as $empresa)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-3 font-medium text-gray-800">{{ $empresa->nome }}</td>
                        <td class="px-5 py-3 text-gray-600 font-mono">{{ $empresa->cnpj }}</td>
                        <td class="px-5 py-3 text-gray-600">{{ $empresa->user?->email ?? '—' }}</td>
                        <td class="px-5 py-3 text-center">
                            <button wire:click="toggleAtivo({{ $empresa->id }})"
                                    wire:confirm="{{ $empresa->user?->active ? 'Desativar acesso desta empresa?' : 'Reativar acesso desta empresa?' }}"
                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium transition-colors
                                    {{ $empresa->user?->active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $empresa->user?->active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                {{ $empresa->user?->active ? 'Ativo' : 'Inativo' }}
                            </button>
                        </td>
                        <td class="px-5 py-3 text-right flex items-center justify-end gap-1">
                            <a href="{{ route('admin.empresas.show', $empresa) }}"
                               class="text-gray-500 hover:text-blue-600 transition-colors p-1 rounded" title="Vincular alunos">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244" /></svg>
                            </a>
                            <button wire:click="openModal({{ $empresa->id }})"
                                    class="text-gray-500 hover:text-senai transition-colors p-1 rounded" title="Editar">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z" /></svg>
                            </button>
                            <button @click="$store.dialogo.perguntar('Excluir empresa', 'O acesso ao sistema será revogado e todos os vínculos com alunos serão removidos.', 'Excluir', () => $wire.excluir({{ $empresa->id }}))"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-10 text-center text-gray-400">Nenhuma empresa encontrada.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md" @click.stop>
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h3 class="font-semibold text-gray-800">{{ $editingId ? 'Editar Empresa' : 'Nova Empresa' }}</h3>
                ...
                <form wire:submit="save" class="px-6 py-5 space-y-4">
                    @if(!$editingId)
                        <div class="px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-xs">
                            Um usuário de acesso será criado automaticamente com senha provisória <strong>senai@empresa</strong>.
                        </div>
                    @endif

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nome da empresa</label>
                        <input wire:model="nome" type="text" placeholder="Razão social"
                    ...
                    <div>
                        ...
                        <input wire:model="cnpj" type="text" placeholder="00.000.000/0000-00"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-senai @error('cnpj') border-red-400 @enderror">
                    ...
                    <div>
                        ...
                        <input wire:model="email" type="email" placeholder="acesso@empresa.com.br"
